 ...
 */
  ...
  PCurve<T,n>::PCurve( const PCurve<T,n>& copy ) : Parametrics<T,1,n>( copy ), _visu(1), _der_implemented(copy._der_implemented) {
    ...
    _local_pre_eval      = copy._local_pre_eval;
...
//      delete _default_visualizer;
  ...
  DVector<Vector<T,n>>& PCurve<T,n>::evaluateParent( T t, int d ) const {
    ...
    Point<T,3> sc =  this->_scale.getScale();
  ...
  /*! Point<T,n>  PCurve<T,n>::getSamplePoint(int i, int j) const
  ...
  Point<T,n>  PCurve<T,n>::getSamplePoint(int i, int j) const {
    ...
    return _visu[1].sample_val[_ind_trans_pre[j]+i][0];
  }


  /*! Vector<T,n>& PCurve<T,n>::getSampleDerivative(int i, int j, int d) const
  ...
  Vector<T,n>&  PCurve<T,n>::getSampleDerivative(int i, int j, int d) const {
    ...
    return _visu[1].sample_val[_ind_trans_pre[j]+i][d];
  }



  /*! int PCurve<T,n>::getSample_d(int i, int j) const
  ...
  int PCurve<T,n>::getSample_d(int i, int j) const {
    ...
    return _visu[1].sample_val[_ind_trans_pre[j]+i].getDim();
  }
  ...
  void PCurve<T,n>::sample( int m, int d ) {
    ...
    if(_local_pre_eval && _visu.size() < 2)
          _visu.resize(2);

    _checkSampleVal( m, d );

    if((_local_pre_eval && _visu.size() > 2) || (!_local_pre_eval && _visu.size()>1) ) std::cerr << "Error, more than 1 partition in simple curve!!";
  ...
  void PCurve<T,n>::setDomainScale( T sc ) {
    ...
    /*! void PCurve<T,n>::computeSurroundingSphere( const std::vector< DVector< Vector<T,n> > >& p, Sphere<T,n>& s ) const
    ...
    /*! void PCurve<T,n>::setLineWidth(double w) const
     *  To set line with to the curve
     *
     *  \param[in]   w   The line with
     */
    template <typename T, int n>
    void PCurve<T,n>::setLineWidth(double w) const {

        for(int i=0; i< _visu.size(); i++) {
            auto ptrvisu = const_cast<GMlib::PCurveVisualizer<T,3>*>(_visu[i].vis[0]);

            GMlib::PCurveDefaultVisualizer<T,3> *visuptr = static_cast<GMlib::PCurveDefaultVisualizer<T,3>*>(ptrvisu);
            visuptr->setLineWidth( w );
        }
    }
    ...
    */
    ...
    void  PCurve<T,n>::cleanVisualizers(unsigned int i) {
        ...
        // Updating visualizers in new partitions
        for( ; i < _visu.size(); i++) {
            while(_visu[i].vis.size()>0) {
                PCurveVisualizer<T,n>* tmp = _visu[i].vis.back();
                _visu[i].vis.pop_back();
